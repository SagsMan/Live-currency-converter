...
:root{
  --g:#16a34a;--gd:#14532d;--gm:#166534;--gl:#dcfce7;--gxl:#f0fdf4;
  --w:#fff;--g1:#f3f4f6;--g2:#e5e7eb;--g5:#6b7280;--g9:#111827;
  --r:#dc2626;--rl:#fee2e2;
}
body{
...
.logo-img{
  ...
  box-shadow:0 3px 12px rgba(0,0,0,.4);background:#fff;padding:3px;
}
.ltd h1{font-size:1.15rem;font-weight:800;color:#fff;letter-spacing:-.02em;line-height:1.2}
.ltd p{font-size:.63rem;color:rgba(255,255,255,.65);text-transform:uppercase;letter-spacing:.07em;margin-top:2px}
.sub{font-size:.7rem;color:rgba(255,255,255,.5);margin-top:.25rem}
.bd{padding:1.75rem 2rem 2rem}
label{display:block;font-size:.7rem;font-weight:700;color:var(--g5);
      text-transform:uppercase;letter-spacing:.07em;margin-bottom:.4rem}
input[type=number],select{
  width:100%;padding:.75rem 1rem;font-size:1rem;color:var(--g9);
  background:var(--g1);border:2px solid var(--g2);border-radius:12px;
  outline:none;appearance:none;transition:border-color .18s,box-shadow .18s;
}
input:focus,select:focus{
  border-color:var(--g);box-shadow:0 0 0 4px rgba(22,163,74,.13);background:var(--w);
}
.fg{margin-bottom:1.1rem}
.pair{display:grid;grid-template-columns:1fr 44px 1fr;gap:.625rem;align-items:end;margin-bottom:1.1rem}
.swap{
  width:44px;height:44px;background:var(--gl);border:2px solid var(--g);
  border-radius:50%;color:var(--gd);font-size:1rem;cursor:pointer;
  display:flex;align-items:center;justify-content:center;transition:background .18s,color .18s;
}
.swap:hover{background:var(--g);color:#fff}
.btn{
  width:100%;padding:.875rem;background:var(--g);color:#fff;
...
.btn:hover{background:#15803d}.btn:active{transform:scale(.98)}
.res{margin-top:1.2rem;border-radius:14px;overflow:hidden}
.ok{background:var(--gxl);border:2px solid var(--g)}
.fail{background:var(--rl);border:2px solid var(--r)}
.rb{padding:1.25rem 1.5rem;text-align:center}
.rfr{font-size:.85rem;color:var(--g5)}
.rarr{font-size:.85rem;color:#9ca3af;margin:.2rem 0}
.rto{font-size:2.1rem;font-weight:800;color:var(--gm);line-height:1.1}
.rto span{font-size:1rem;font-weight:600;color:var(--g)}
.rrate{font-size:.73rem;color:var(--g5);margin-top:.45rem}
.rdate{font-size:.68rem;color:#9ca3af;margin-top:.1rem}
.emsg{padding:1rem 1.5rem;color:var(--r);font-weight:600;font-size:.88rem}
footer{margin-top:1.4rem;text-align:center;color:rgba(255,255,255,.45);font-size:.68rem;z-index:1;position:relative}
footer span{color:rgba(255,255,255,.8)}
</style>
</head>
<body>
